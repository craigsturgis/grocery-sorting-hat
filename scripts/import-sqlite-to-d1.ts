...
if (!USER_EMAIL) {
  console.error("Usage: tsx import-sqlite-to-d1.ts [sqlite-db-path] <user-email>");
  process.exit(1);
}
...
console.log(`Importing data for user: ${USER_EMAIL} (ID: ${userId})`);
...
// Insert user
sqlStatements.push(
  `INSERT INTO users (id, email, name) VALUES ('${userId}', '${USER_EMAIL}', '${USER_EMAIL}');`
);
...
categories.forEach((cat, index) => {
  ...
  sqlStatements.push(
    `INSERT INTO categories (id, user_id, name) VALUES (${newId}, '${userId}', '${cat.name.replace(/'/g, "''")}');`
  );
});
...
items.forEach((item, index) => {
  ...
  sqlStatements.push(
    `INSERT INTO items (id, user_id, name, price, category_id, source, taxable, created_at) VALUES (${newId}, '${userId}', '${item.name.replace(/'/g, "''")}', ${item.price}, ${newCategoryId}, '${item.source}', ${item.taxable}, '${item.created_at}');`
  );
});
...
receipts.forEach((receipt, index) => {
  ...
  sqlStatements.push(
    `INSERT INTO receipts (id, user_id, source, date) VALUES (${newId}, '${userId}', '${receipt.source}', '${receipt.date}');`
  );
});
...
try {
  console.log("Executing import to D1...");
  execSync(`npx wrangler d1 execute ${D1_DATABASE_NAME} --file=${tempSqlFile}`, {
    stdio: "inherit",
  });
